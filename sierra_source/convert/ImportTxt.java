...
public class ImportTxt implements CustomEventInterface {
	
	
	private int idRangeStart = 90000000;
	private int idRangeEnd = 99999999;
	
	public void normalizeTextData(String domainFolder) throws IOException {
		String source1 = FileUtilities.checkDirectoryEnding(domainFolder).concat(PrepareDomain.TXT_CONVERT_FOLDER);
		String source2 = FileUtilities.checkDirectoryEnding(domainFolder).concat(PrepareDomain.TXT_IMPORT_FOLDER);
		String target = FileUtilities.checkDirectoryEnding(domainFolder).concat(PrepareDomain.TXT_CHANGE_FOLDER);
		...
		filesImportPdf.addAll(fu.searchForFiles(source2, ".txt"));
		...
		for (String file : filesImportPdf) {
			...
			if (filesImport.contains(new File(file).getParent())) {
				...
				md.setTxthash(MD5Checksum.getMD5Checksum(file));
		...
		for (String source : sources) {
			files.addAll(fu.searchForFiles(source, ".txt"));
		...
		fireCustomEvent(new CustomEvent(this).setMessage("import done."));
